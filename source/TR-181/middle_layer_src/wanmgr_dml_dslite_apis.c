...
ANSC_HANDLE InterfaceSetting4_GetEntry(ANSC_HANDLE hInsContext, ULONG nIndex, ULONG *pInsNumber)
{
    UNREFERENCED_PARAMETER(hInsContext);
    UINT inst;
    DML_DSLITE_LIST *pDSLiteEntry;

    pDSLiteEntry = WanMgr_getDSLiteCfgByIdx_locked(nIndex);
    if (!pDSLiteEntry)
    {
        return NULL;
    }

    *pInsNumber = pDSLiteEntry->InstanceNumber;
    inst = pDSLiteEntry->InstanceNumber;
...
BOOL InterfaceSetting4_Validate(ANSC_HANDLE hInsContext, char *pReturnParamName, ULONG *puLength)
{
    ...
    DML_DSLITE_LIST *pThis = WanMgr_getDSLiteCfgByInstance_locked(inst);
    if (!pThis)
        return FALSE;

    DML_DSLITE_CONFIG *cfgThis = &pThis->CurrCfg;

    if (cfgThis->Alias[0] != '\0')
    {
        pDSLiteData = &gWanMgrDataBase.DSLite;
        DML_DSLITE_LIST *pcfg = pDSLiteData->DSLiteList;
        ...
        while (pcfg)
        {
            if (pcfg != pThis)
            {
                DML_DSLITE_CONFIG *cfgOther = &pcfg->CurrCfg;
