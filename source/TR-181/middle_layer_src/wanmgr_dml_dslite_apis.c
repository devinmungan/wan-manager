...
ULONG InterfaceSetting4_DelEntry(ANSC_HANDLE hInsContext, ANSC_HANDLE hInstance)
{
    ...
    if (WanMgr_DSLite_RemoveFromList(inst) != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to remove DSLite entry from list\n", __FUNCTION__));
        return ANSC_STATUS_FAILURE;
    }

    if (WanMgr_DSLite_DeleteEntryConfig(inst) != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to delete DSLite entry config\n", __FUNCTION__));
        return ANSC_STATUS_FAILURE;
    }

    if (WanMgr_DSLite_SaveMainConfig() != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to save DSLite main config after delete\n", __FUNCTION__));
        return ANSC_STATUS_FAILURE;
    }
...
ULONG InterfaceSetting4_Commit(ANSC_HANDLE hInsContext)
{
    ...
    if (WanMgr_DSLite_SaveMainConfig() != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to save DSLite main config\n", __FUNCTION__));
        ret = ANSC_STATUS_FAILURE;
        goto exit;
    }

    if (WanMgr_DSLite_SaveEntryConfig(inst) != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to save DSLite entry config\n", __FUNCTION__));
        ret = ANSC_STATUS_FAILURE;
        goto exit;
    }

    if (WanMgr_DSLite_HandleConfigChange(inst) != ANSC_STATUS_SUCCESS)
    {
        CcspTraceError(("%s: Failed to handle DSLite config change\n", __FUNCTION__));
        ret = ANSC_STATUS_FAILURE;
        goto exit;
    }
