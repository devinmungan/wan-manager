...
#ifdef DSLITE_FEATURE_SUPPORT_V2
...
ANSC_STATUS WanMgr_DSLite_WriteEntryCfgToSyscfg(UINT inst)
{
    ...
    DSLITE_WRITE_IF_CHANGED(entry->New, WanMgr_SysCfgSetUint(key, inst));

    snprintf(key, sizeof(key), "dslite_active_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->Enable != curr->Enable,
                     WanMgr_SysCfgSetUint(key, curr->Enable ? 1 : 0));

    snprintf(key, sizeof(key), "dslite_alias_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || strcmp(prev->Alias, curr->Alias) != 0,
                     WanMgr_SysCfgSetStr(key, curr->Alias));

    snprintf(key, sizeof(key), "dslite_mode_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->Mode != curr->Mode,
                     WanMgr_SysCfgSetUint(key, curr->Mode));

    snprintf(key, sizeof(key), "dslite_addr_type_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->Type != curr->Type,
                     WanMgr_SysCfgSetUint(key, curr->Type));

    snprintf(key, sizeof(key), "dslite_addr_fqdn_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || strcmp(prev->AddrFqdn, curr->AddrFqdn) != 0,
                     WanMgr_SysCfgSetStr(key, curr->AddrFqdn));

    snprintf(key, sizeof(key), "dslite_addr_ipv6_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || strcmp(prev->AddrIPv6, curr->AddrIPv6) != 0,
                     WanMgr_SysCfgSetStr(key, curr->AddrIPv6));

    snprintf(key, sizeof(key), "dslite_mss_clamping_enable_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->MssClampingEnable != curr->MssClampingEnable,
                     WanMgr_SysCfgSetUint(key, curr->MssClampingEnable ? 1 : 0));

    snprintf(key, sizeof(key), "dslite_tcpmss_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->TcpMss != curr->TcpMss,
                     WanMgr_SysCfgSetUint(key, curr->TcpMss));

    snprintf(key, sizeof(key), "dslite_ipv6_frag_enable_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || prev->Ipv6FragEnable != curr->Ipv6FragEnable,
                     WanMgr_SysCfgSetUint(key, curr->Ipv6FragEnable ? 1 : 0));

    snprintf(key, sizeof(key), "dslite_tunnel_v4addr_%lu", inst);
    DSLITE_WRITE_IF_CHANGED(entry->New || strcmp(prev->TunnelV4Addr, curr->TunnelV4Addr) != 0,
                     WanMgr_SysCfgSetStr(key, curr->TunnelV4Addr));
...
{
    WanMgr_DSLite_Data_t *pDSLiteData = WanMgr_GetDSLiteData_locked();
    if (!pDSLiteData)
    {
        return ANSC_STATUS_FAILURE;
    }
...
ANSC_STATUS WanMgr_DSLite_DelFromList(UINT inst)
{
    ...
    DML_DSLITE_LIST *curr = pDSLiteData->DSLiteList;
...
DML_DSLITE_LIST *WanMgr_getDSLiteCfgByInstance_locked(UINT inst)
{
    if (pthread_mutex_lock(&gWanMgrDataBase.gDataMutex) == 0)
    {
        ...
        while (pCfg != NULL)
        {
            if (pCfg->InstanceNumber == (ULONG)inst)
            {
                return pCfg;
            }
            pCfg = pCfg->next;
...
DML_DSLITE_LIST *WanMgr_getDSLiteCfgByIdx_locked(UINT idx)
{
    if (pthread_mutex_lock(&gWanMgrDataBase.gDataMutex) == 0)
    {
        UINT i = 0;
